...
std::string Mkfs(const std::string& id, const std::string& type) {
    ...
    //  3. Calcular n 
    // La fórmula :
    //   n = floor(partSize / (sizeof(SB) + 4 + 4*sizeof(Inode) + 3*sizeof(Bloque)))
    // De dónde sale:
    //   sizeof(SB)         -> el superbloque en sí
    //   1                  -> 1 byte en bitmap_inodos por inodo
    //   3                  -> 3 bytes en bitmap_bloques por inodo (hay 3n bloques)
    //   4*sizeof(Inode)    -> cada inodo ocupa este espacio en la tabla
    //   3*sizeof(FolderBlock) _> cada inodo tiene 3 bloques asociados (64B c/u)
    //
    // Resultado: n = cantidad de inodos (y también bloques = 3n)

    int n = partSize / (sizeof(SuperBloque) + 4 +
                        4 * sizeof(Inode) +
                        3 * sizeof(FolderBlock));

    if (n < 2) {
        out << "Error: La partición es demasiado pequeña para EXT2\n";
        file.close();
        return out.str();
    }

    out << "Calculando estructuras EXT2:\n";
    out << "  n (inodos) = " << n << "\n";
    out << "  Bloques    = " << 3 * n << "\n";

    // 4. Calcular posiciones absolutas en el disco 
    //
    // Todo se calcula como OFFSET desde el inicio del disco (byte 0),
    // no desde el inicio de la partición.
    //
    // Layout en el disco:
    //   [partStart + 0] -> SuperBloque
    //   [partStart + sizeof(SB)]  -> Bitmap Inodos (n bytes)
    //   [partStart + sizeof(SB) + n] -> Bitmap Bloques (3n bytes)
    //   [partStart + sizeof(SB) + 4n] -> Tabla de Inodos (n × sizeof(Inode))
    //   [partStart + sizeof(SB) + 4n + n×sizeof(Inode)] -> Tabla de Bloques

    int bm_inode_start = partStart + sizeof(SuperBloque);
    int bm_block_start = bm_inode_start + n;
    int inode_start    = bm_block_start + 3 * n;
    int block_start    = inode_start + n * sizeof(Inode);
    ...
    sb.s_umtime            = 0;
    sb.s_mnt_count         = 1;
    sb.s_magic             = 0xEF53; // número mágico EXT2
    ...
    // n bytes, uno por inodo. Bit = 0 libre, Bit = 1 ocupado.
    // Escribimos byte a byte porque cada byte representa 8 inodos.
    {
        char zeroByte = 0;
        for (int i = 0; i < n; i++) {
    ...
    // Escribimos n inodos, todos con i_block[x] = -1 (sin bloques)
    {
        Inode emptyInode{};
        for (int i = 0; i < 15; i++) emptyInode.i_block[i] = -1;
        emptyInode.i_uid = -1;
        emptyInode.i_gid = -1;
        emptyInode.i_size = 0;
    ...
    // Bloques de carpeta con b_inodo = -1 (entradas vacías)
    {
    ...
    // NOTA: Los FileBlock y FolderBlock comparten la tabla de bloques y tienen el mismo tamaño (64 bytes),así que usamos el mismo offset.
