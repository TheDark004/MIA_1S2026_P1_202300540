...
static std::string ValidateParams(const std::string& command,
    ...
    std::string Analyze(const std::string &input)
    {
        ...
        for (; fit != fend; ++fit) {
            ...
            if (!params.count(flag)) flags[flag] = true;
        }
        ...
        if (command == "mkdisk") {
            ...
            if (!err.empty()) return err;
            if (!params.count("path")) return "Error RMDISK: Se requiere -path\n";
    ...
    // ── FILESYSTEM ─────────────────────────────────────────────
            ...
            err = ValidateParams("MKFS", params, {"id","type"});
            if (!err.empty()) return err;
            if (!params.count("id")) return "Error MKFS: Se requiere -id\n";
    ...
    // ── SESIÓN ─────────────────────────────────────────────────
        ...
        // ── ARCHIVOS ───────────────────────────────────────────────
            ...
            // Acepta -file o -file1
            std::string filePath;
            if      (params.count("file"))  filePath = params["file"];
            else if (params.count("file1")) filePath = params["file1"];
            else return "Error CAT: falta -file o -file1\n";
        ...
        if (!err.empty()) return err;
        if (!params.count("id"))   return "Error REP: falta -id\n";
        if (!params.count("name")) return "Error REP: falta -name\n";
 
        // Acepta -path= o -ruta=
        std::string ruta;
        if      (params.count("path")) ruta = params["path"];
        else if (params.count("ruta")) ruta = params["ruta"];
        else return "Error REP: falta -path\n";
 
        // Para reportes file y ls → acepta -path_file_ls=
        std::string filePath;
        if (params.count("path_file_ls")) filePath = params["path_file_ls"];
